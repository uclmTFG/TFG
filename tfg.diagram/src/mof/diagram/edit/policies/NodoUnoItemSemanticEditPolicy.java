...
import mof.diagram.edit.commands.EnlaceCreateCommand;
import mof.diagram.edit.commands.EnlaceReorientCommand;
 ...
 * @generated
 */
public class NodoUnoItemSemanticEditPolicy extends MofBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public NodoUnoItemSemanticEditPolicy() {
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (MofVisualIDRegistry.getVisualID(incomingLink) == EnlaceEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (MofVisualIDRegistry.getVisualID(outgoingLink) == EnlaceEditPart.VISUAL_ID) {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (MofVisualIDRegistry.getVisualID(node)) {
			case NodoUnoNodoUnoNodosDosCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (MofVisualIDRegistry.getVisualID(cnode)) {
	...
	protected Command getStartCreateRelationshipCommand(CreateRelationshipRequest req) {
		if (MofElementTypes.Enlace_4001 == req.getElementType()) {
