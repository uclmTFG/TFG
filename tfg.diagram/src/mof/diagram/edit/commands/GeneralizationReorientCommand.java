...
public class GeneralizationReorientCommand extends EditElementCommand {
	...
	*/
	protected boolean canReorientSource() {
		if (!(oldEnd instanceof Nodeone && newEnd instanceof Nodeone)) {
			return false;
		}
		Nodeone target = getLink().getTarget();
		if (!(getLink().eContainer() instanceof UMLDiagram)) {
			return false;
		}
		UMLDiagram container = (UMLDiagram) getLink().eContainer();
		return MofBaseItemSemanticEditPolicy.getLinkConstraints().canExistGeneralization_4001(container, getLink(),
				getNewSource(), target);
	}
	...
	*/
	protected boolean canReorientTarget() {
		...
		Nodeone source = getLink().getSource();
		if (!(getLink().eContainer() instanceof UMLDiagram)) {
			return false;
		}
		UMLDiagram container = (UMLDiagram) getLink().eContainer();
	...
	* @generated
	*/
	protected Nodeone getOldSource() {
		return (Nodeone) oldEnd;
	}

	/**
	* @generated
	*/
	protected Nodeone getNewSource() {
		return (Nodeone) newEnd;
	}

	/**
	* @generated
	*/
	protected Nodeone getOldTarget() {
		return (Nodeone) oldEnd;
	}
	...
	* @generated
	*/
	protected Nodeone getNewTarget() {
		return (Nodeone) newEnd;
	}
}
