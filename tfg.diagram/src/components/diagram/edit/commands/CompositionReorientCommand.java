...
public class CompositionReorientCommand extends EditElementCommand {
	...
	*/
	protected boolean canReorientSource() {
		...
		Classifier target = getLink().getTarget();
		if (!(getLink().eContainer() instanceof UMLDiagram)) {
			return false;
		}
		UMLDiagram container = (UMLDiagram) getLink().eContainer();
		return ComponentsBaseItemSemanticEditPolicy.getLinkConstraints().canExistComposition_4005(container, getLink(),
				getNewSource(), target);
	}
	...
	*/
	protected boolean canReorientTarget() {
		...
		Classifier source = getLink().getSource();
		if (!(getLink().eContainer() instanceof UMLDiagram)) {
			return false;
		}
		UMLDiagram container = (UMLDiagram) getLink().eContainer();
